...
- **当前状态**：`en-US`、`zh-CN`、`ja-JP` 各 37 篇（入门 4 + 概念 9 + 应用 24）且完全对齐。**增删内容文件时必须让三种语言的篇数保持一致**，否则导航会出现回退项。改完跑 `node tools/verify-doc-order.mjs`（篇数 / `order` 唯一性 / 三语对齐）与 `node tools/verify-doc-links.mjs`（内链与 front matter）复核。
